...
def bpkewedsticky():
    ...
    s5=0
    s1=0
    with open('arrivalrateskewedsticky.csv', 'r') as csvfile:
    ...
    with open('consumptionrateskewedstikcy.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            #t.append(math.floor(double(row[0])))
            #t.append(str(row[0]))
    ...
    for r in range(len(t1)):
           print(r, t1[r])
           print(len(t1))
    with open('repskewedbp.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            #t.append(math.floor(double(row[0])))
            #t.append(str(row[0]))
    ...
    #
    plt.title('Skewed Workload with non load-aware (Incremental Sticky) assignment')
    ax1.legend()
    ax2.legend(loc =4 )
    #
    plt.show()
...
def lagskewedbpsticky():
    ...
    s5=0
    s1=0
    with open('lagskewedstikcy.csv', 'r') as csvfile:
        plots = csv.reader(csvfile, delimiter=',')
        for row in plots:
            #t.append(math.floor(double(row[0])))
            #t.append(str(row[0]))
    ...
    # #ax2.set_ylabel('Consumer replicas')
    #
    plt.title('Event Lag (Skewed Workload with non load-aware -Incremental Sticky- assignment')
...
def workload():
        ...
        # #ax2 = ax1.twinx()
        # ax1.plot(t5, lamda5, color='blue', label='lag')
        ax1.plot(t15, lamda15, color='red', label='Event arrivals per second')
        # ax1.plot(t1, lamda1, color='black', label='workload1')
        ...
        ax1.set_xlabel('Time (sec)')
        ax1.set_ylabel('Arrival Rate per Seconds')
        ax1.legend()
        # #ax2.legend(loc=1)
        #
